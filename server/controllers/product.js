...
//     const products = await Product.find({}).populate("category").populate("subs").sort([[sort, order]]).limit(limit).exec();
...
exports.productStar = async (req, res) => {
  ...
  // who is updating
  // check if user has already added the rating to this product

  let existingRatingObject = product.ratings.find((ele) => (ele.postedBy.toString() === user._id.toString()));

  // if user haven't added the rating, push it
  if (existingRatingObject === undefined) {
    ...
    // if user has already added the rating, update it
